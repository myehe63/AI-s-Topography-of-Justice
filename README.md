...
Intentions and outcomes are represented as vectors in a shared moral space.
Angular distance between vectors is used to diagnose how cases deviate from a reference direction.

| Vector | Description |
| --- | --- |
| **Ideal Vector** | A reference direction derived from collective moral intuition. Used as a baseline for comparison. |
...
![figure1.png](attachment:32dd73e1-0d5e-49aa-9a38-63fad116db20:figure1.png)
...
The two Claude agents reinforced each other's positions noticeably. This likely reflects RLHF training values surfacing in debate — rather than genuine emergent polarization.
...
## 5. What the Data Suggests So Far

**1. Dilemma type shapes how opinions cluster**

- Trolley problem: K=6 clusters, max angle 5.3° — opinions spread relatively evenly
- Anthropic/DoD case: K=2 clusters, angle 10.7–11.9° — a sharper split

**2. Axis choice affects how much variation is visible**

- General axes (fairness/survival/autonomy): 2.2° — differences too small to read clearly
- Domain-specific axes (state_authority/corporate_ethics/civilian_safety): 11.9° — conflict becomes visible

**3. LLM personas may compress moral variance**
RLHF fine-tuning likely pulls model responses toward median values, which could reduce the spread of opinion seen in actual human populations. This pattern is consistent across all three models tested here, though the sample is small.

**4. Multi-agent debate may produce more distributed results**
Debate outputs were closer to the ITIF survey than standalone LLM simulation (6.2° vs 10.0° gap). Mutual rebuttal may partially offset the median-compression effect — though this is tentative given the limited number of rounds.

**5. Model baselines differ across companies**
Same dilemma, different starting points: Claude scored state_authority at 20, GPT at 30–40, Gemini at 40–45. This could reflect differences in training approach, though other explanations aren't ruled out. Adding Grok would be a useful next test.
